...
.addEventListener(
    ...
    async function(){
        ...
        // ログインユーザーのデータだけをローカルから削除
        // user_id が null のデータは残す
        birds =
...
async function saveBirdToSupabase(bird) {
    ...
    // Supabaseで自動生成されたidをBird Map側にも保存する
    bird.id = data.id;

    return true;
}

async function loadBirdsFromSupabase() {
...
marker.on("mouseout", function() {
    ...
    // Supabaseに保存されているデータなら
    // Supabaseからも削除する
if (bird.user_id) {

    console.log("削除するbird:", bird);
    console.log("削除するid:", bird.id);
    ...
    console.log("Supabaseで削除されたデータ:", data);

    if (!data || data.length === 0) {
        alert(
            "Supabaseの削除対象が見つかりませんでした。\n" +
            "id = " + bird.id
        );
